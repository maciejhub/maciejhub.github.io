	...
	if (done.includes(name) == false) {
		...
		if (name == "zepsucnokie") {
			document.getElementById(name).height =
				document.getElementById(name).scrollHeight + 300;
		} else {
			document.getElementById(name).height =
				document.getElementById(name).scrollHeight + 250;
		}
	}
	done.push(name);
}
...
if (
	localStorage.quest == "nRM" ||
	localStorage.quest == "nZNT" ||
	localStorage.quest == "nAS" ||
  localStorage.quest == "nKRT" ||
  localStorage.quest == "nZNS"
) {
...
if (shared != null || shared2 != null) {
	if (localStorage.shareVisits) {
		localStorage.setItem(
			"shareVisits",
			parseInt(localStorage.shareVisits) + 1,
		);
	} else {
...
if (shared != null) {
	if (shared != "all") {
		document.getElementById("showallbutton").style.display =
			"initial";
		document.getElementById("maciejclicker").style.display =
			"none";
		document.getElementById("skibidi").style.display = "none";
		document.getElementById("zepsucnokie").style.display =
			"none";
		document.getElementById("mandarynki").style.display =
			"none";
		document.getElementById("asmrbulka").style.display = "none";
		document.getElementById("nokiatycoon").style.display =
			"none";
...
async function checkcode() {
	...
	console.log(offset);
	if (
		isNumber(offset) == false ||
		isNumber(code.substring(1, 2)) ||
		isNumber(code.substring(3, 4)) ||
		isNumber(code.substring(2, 3)) == false ||
		isNumber(code.substring(4, 5)) == false
	) {
	...
	madecodearray.push(
		lettertonumber.indexOf(code.substring(1, 2)) - offset,
	);
	if (madecodearray[2] < 0) {
		madecodearray[2] += 9;
	}
	madecodearray.push(
		lettertonumber.indexOf(code.substring(3, 4)) - offset - 2,
	);
	...
	let madecode =
		madecodearray[0].toString() +
		madecodearray[1].toString() +
		madecodearray[2].toString() +
		madecodearray[3].toString();
...
if (document.getElementById("sharechoose").value == "all") {
	...
	shareurl =
		"https://maciejhub.github.io/gry?share=" +
		document.getElementById("sharechoose").value;
}
...
function changesorting() {
  ...
  if (sorting == 1) {
    ...
    get("8").appendChild(get("zntr"));
    get("sorting_button").innerText = "Sortowanie: Data wydania";
  } else if (sorting = 3) {
    ...
    get("sorting_button").innerText = "Sortowanie: Czas tworzenia";
  }
}
